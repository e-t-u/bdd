...
- **The Solution: Field Fusion inside `rearrange(...)` and Concise Tuple Projector `{...}`**:
  Instead of requiring a long keyword like `rearrange(...)`, we specify two interchangeable syntaxes:
  
  1. **Concise Tuple Projector Notation `{...}` (Recommended)**:
     Using curly braces `{...}` as a first-class pipeline stage to construct and project tuples:
     ```bash
     {0, 1|2}
     ```
     - **`"4U4U -> {0|1} -> 16U -> hex"`**: Fuses two 4-bit fields into one 8-bit field, producing `00fa`!
     - **`"4U4U4U -> {0, 1|2} -> hex"`**: Keeps field 0, fuses fields 1 and 2.
     - **`"tag:4U,hi:4U,lo:8U -> {tag, hi|lo} -> json"`**: Clean named-field projection and fusion.
     - **`"4U4U -> {1, 0} -> hex"`**: Concise field swap (no fusion).
     - **`"4U4U4U -> {0} -> hex"`**: Keeps field 0, discards fields 1 and 2.

  2. **Verbose Functional Notation `rearrange(...)`**:
     For CLI flag consistency (`--rearrange="0, 1|2"`) and backward compatibility:
     ```bash
     rearrange(0, 1|2)
     ```
  ...
  - **Why `|` is the Ideal Delimiter**:
    ...
    2. Inside `{...}` or `rearrange(...)`, there is no arithmetic, making `|` completely unambiguous.
    3. Curly braces `{...}` have established precedent in hardware languages (Verilog bit concatenation `{a, b}`) and relational algebra (tuple projection).
...
ManipStage      ::= ManipName "(" [ ManipArgs ] ")" | "not" | "abs" | "sign" | TupleProjector
TupleProjector  ::= "{" ProjectorItem ("," ProjectorItem)* "}"
ProjectorItem   ::= FieldRef ("|" FieldRef)*
FieldRef        ::= Integer | Identifier
...
| **Field Glueing & Packing** | *(Requires multi-step bit shift math)* | `"stdin -> 8 -> 4U4U -> rearrange(0|1) -> 16U -> 16 -> hex"` | `"4U4U -> {0|1} -> 16U -> hex"` |
...
**Answer**: **Yes.** Enhancing `rearrange(...)` with the bitwise OR symbol `|` unifies field selection, reordering, dropping, and bitwise concatenation into a single operator.

### Q6: `rearrange` is quite a long word. Could there be an alternative notation like `{0, 1|2}`?
**Answer**: **Yes! The `{0, 1|2}` tuple projector syntax is significantly better and adopted as primary syntax.**
- Instead of writing `rearrange(0, 1|2)`, the pipeline stage uses curly braces:
  $$\text{\texttt{"4U4U -> \{0\|1\} -> 16U -> hex"}}$$
  $$\text{\texttt{"4U4U4U -> \{0, 1\|2\} -> hex"}}$$
  $$\text{\texttt{"tag:4U,hi:4U,lo:8U -> \{tag, hi\|lo\} -> json"}}$$
- **Advantages**:
  1. **Zero keyword overhead**: Instant recognition as a tuple constructor / record projector.
  2. **Hardware precedent**: Matches Verilog / SystemVerilog bit-concatenation vector syntax `{a, b}`.
  3. **Zero ambiguity**: Curly braces `{}` are completely unused elsewhere in `bdd` grammar (slicers use `[]`, multipliers use `()`).
  4. **Coexistence**: `rearrange(...)` remains supported as the verbose functional equivalent for CLI flag symmetry (`--rearrange="0, 1|2"`).
