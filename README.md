...
### The Solution: Single-Bit Stream Transformation
`bdd` brings sub-byte surgical precision to the Unix pipeline. What previously required complex C structs or slow Python glue scripts now happens in a single, intuitive stream arrow expression:

```bash
# Slices two 4-bit nibbles, fuses them into an 8-bit byte, and formats as hex:
echo -ne "\xfa" | bdd "4U4U -> {0|1} -> 8U -> hex"
# Output: fa

# Swap high and low 4-bit nibbles on the fly:
...
# Calibrate 12-bit ADC sensor data, remove DC bias, amplify, and clamp to 8-bit DAC:
...
| `netlink` | Live Linux kernel process lifecycle events (`AF_NETLINK` `CN_IDX_PROC`) | `sudo bdd "netlink -> json"` |
| `file(path)` | Direct file read without shell redirection | `bdd "file('dump.bin') -> 8 -> hex"` |
| `tuples` | Reads comma-separated text tuple lines from standard input | `bdd "tuples -> 8U8U -> stdout"` |
...
Active fields rarely float in empty space; they reside inside physical containers (bytes, 16-bit halfwords, 32-bit registers, 188-byte MPEG packets).
...
Here is `bdd` in action across six high-impact systems engineering and data pipeline workflows:
...
## 6. Native Linux Netlink Kernel Telemetry vs. Python Analytics
...
└───────────────────────────────────┬────────────────────────────────────┘
                                    ...
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│               Tier 1: Native Rust High-Speed Ingestion                 │
│                 sudo bdd "netlink -> json" (or CLI flags)              │
...
### 1. Tier 1: Native Rust Kernel Ingestion (`bdd "netlink -> ..."`)
Run `bdd` with root privileges to connect directly to the kernel process connector:

```bash
# Ingest kernel process events directly into structured JSON:
sudo bdd "netlink -> json"

# Or using dedicated CLI flags:
sudo bdd --input-netlink --output-json

# Unpack specific proc_event header fields (Timestamp, CPU ID, Event Code):
sudo bdd --input-netlink "timestamp_ns:64U,cpu:32U,what:32U" --output-tuples
```

### 2. Tier 2: Python Analytics & SIEM Enrichment (`bdd_netlink_proc.py`)
While `bdd` in Rust operates as an ultra-high-speed, zero-copy packet ingestion engine, security operations centers (SOC) and site reliability engineers (SRE) require stateful correlation and command-line resolution:
...
## 7. Non-8-Bit-Aligned Bitstreams in the CLI & Engine
...
## 8. Multi-Stream Coordination: Merging, Interleaving & Demuxing

### Round-Robin Stream Merging (`--merge-file`)
`bdd` can interleave data from one or more secondary files round-robin with the primary stream:

```bash
# Interleave 32-bit counter addresses with 8-bit payload bytes:
...
```bash
# Instantly seek 10 GiB into a raw disk image and inspect 4 bytes:
bdd --input-file=disk.img --input-skip-bits=10GiB --count=4 --output-hex
...
Instead of manually typing complex bit patterns, 19 standard protocol presets configure patterns, unit widths, and field names in one step:
...
```bash
# Inspect MPEG-TS headers as structured JSON objects:
bdd --input-file stream.ts --input-raw-unit 188B --preset mpeg-ts --json-object --count 3
```

### Pattern Explainer & Struct Code Generation (`--explain-pattern`, `--export-c`, `--export-rust`)
Inspect bit layouts or generate production-ready packed C structs and Rust struct definitions:
...
`bdd` includes a native JSON-RPC 2.0 stdio MCP server for direct integration with AI coding assistants (Claude Desktop, Cursor, Antigravity):
...
For AI autonomous agents, programmatic wrapper scripts, and legacy workflows, all stream arrow parameters can also be configured via explicit CLI flags:
...
- **Fedora / RHEL / CentOS / Rocky (`.rpm` for DNF)**:
  ```bash
  make rpm
  sudo dnf install ./dist/bdd-0.5.2-1.*.rpm
  ```
- **Debian / Ubuntu / Linux Mint (`.deb`)**:
  ```bash
  make deb
  sudo apt install ./dist/bdd_0.5.2_amd64.deb
  ```
- **Rust Cargo Crate**:
  ```bash
...
- **Python Module via Pip**:
  ```bash
  make python
  pip install ./dist/bdd-0.5.2-py3-none-any.whl
  ```
- **C SDK Archive**:
  ```bash
  make c-lib
